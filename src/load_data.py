...
def load_ev_data(filename="Kaggle-EV-Dataset.csv"):
    ...
    df['delta_odo'] = df.groupby('COND')['ODO'].diff().fillna(0.001)  # avoid divide by zero
    df['road_grad'] = df['delta_alt'] / df['delta_odo']
    df['power'] = df['VOL'] * df['CUR']
    rolling_window = 5
    df['acc_mag_roll'] = df.groupby('COND')['acc_mag'].rolling(window=rolling_window, min_periods=1).mean().reset_index(0, drop=True)
    df['spd_roll'] = df.groupby('COND')['SPD'].rolling(window=rolling_window, min_periods=1).mean().reset_index(0, drop=True)
